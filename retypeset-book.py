#!/usr/bin/env python3
"""Setzt von einfachvorlesen.de geladene Bücher neu — warum so, steht in README.md.
...
Die Quellen wandern danach nach processed/, das fertige Buch nach retypeset/.
...
# Das Werkzeug wohnt bei seinen Büchern: neben dem Skript liegen die vier
# Ordner, durch die ein Buch wandert. Keiner davon ist versioniert — für
# die Geschichten gibt es keine Lizenz zum Weitergeben.
ROOT = Path(__file__).resolve().parent
FONT_DIR = ROOT / "fonts"
WORK_ROOT = ROOT / "work"  # Arbeitsordner je Buch: buch.json, bilder/
PROCESSED_DIR = ROOT / "processed"  # ausgelesene Quell-PDFs
RETYPESET_DIR = ROOT / "retypeset"  # fertig gesetzte Bücher
...
def render_typst(recipe) -> str:
    ...
    lines = [
        "// Erzeugt von retypeset-book.py — nicht von Hand ändern.",
        ...
        "// besser als Blocksatz. Der optimierte Umbruch verteilt die",
    ...
    # Vorlese-App (github.com/DrDonik/super-vorlese-app, ADR 29 dort) liest
    # Coverbild und Altersempfehlung aus Seite 1 des fertigen PDF, statt ein
    # Vollseiten-Thumbnail zu nehmen. Sie erkennt beides daran, dass die
    # Seite genau ein Rasterbild trägt und die Altersangabe („Ab 5 Jahren“)
    # als Text enthält. Ein zweites Bild hier oder eine Altersangabe, die zur
    # Grafik wird, nimmt dem Regal dort still die Cover — auffallen würde das
    # erst beim nächsten Import eines neu gesetzten Buchs.
    lines += [
...
def build(work_dir, preview=False):
    ...
    print(f"  -> {final_pdf.relative_to(ROOT)}")
...
def main():
    ...
    parser.add_argument(
        ...
        "--out",
        type=Path,
        help="Arbeitsordner für die Quell-PDFs (Vorgabe: work/<titel>)",
    )
    parser.add_argument(
    ...
    print(f"  Quellen: {PROCESSED_DIR.relative_to(ROOT)}")
